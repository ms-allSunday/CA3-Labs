...
void CS43L22_Beep(soundToneType pitch, uint32_t duration_ms)
{
	...
	uint16_t led_pin = 0;
	if (pitch == C4 || pitch == C5 || pitch == C6 || pitch == C7) led_pin = GPIO_PIN_12;
	    else if (pitch == D5 || pitch == D6) led_pin = GPIO_PIN_13;
	    else if (pitch == E5 || pitch == E6) led_pin = GPIO_PIN_14;
	    else if (pitch == F5 || pitch == F6) led_pin = GPIO_PIN_15;
	    else if (pitch == G5 || pitch == G6) led_pin = GPIO_PIN_12 | GPIO_PIN_13;
	    else if (pitch == A5 || pitch == A6) led_pin = GPIO_PIN_14 | GPIO_PIN_15;
	    else if (pitch == B5 || pitch == B6) led_pin = GPIO_PIN_12 | GPIO_PIN_15;

	    if (led_pin != 0) HAL_GPIO_WritePin(GPIOD, led_pin, GPIO_PIN_SET);
	// Playing...
